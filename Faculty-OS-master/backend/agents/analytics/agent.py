...
def handle_analytics_chat(message: str, faculty_id: int, db: Session, history: list = None):
    ...
    if "at risk" in msg_lower or "risk" in msg_lower or "weak" in msg_lower:
        ...
        try:
            # Rule-based prediction: attendance < 75% or total_marks < 25/50
            ...
            if not results:
                # Fallback to mock student if none found
                results = [{"name": "A. Kumar", "roll_no": "24CC001", "attendance": 40, "marks": 37, "reason": "Attendance below 75% threshold"}]
        ...
        try:
            w_records = db.query(FacultyWorkload).all()
            total_hours = sum([w.weekly_hours for w in w_records]) if w_records else 10
