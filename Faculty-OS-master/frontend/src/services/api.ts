...
export const api = {
  ...
  async generatePaper(data: any): Promise<any> {
    try {
      ...
      const subject = data?.subject || 'Design & Analysis of Algorithms';
      const exam_type = data?.exam_type || 'CAT2';
      const total_marks = Number(data?.total_marks || 50);
      const duration = Number(data?.duration || 90);

      const part_a = [
        { id: 101, section: 'Part A', question_number: 1, question_text: 'Define Asymptotic Notation and list its three primary types.', marks: 2, co: 'CO1', bloom_level: 'Remember', unit: 1 },
        { id: 102, section: 'Part A', question_number: 2, question_text: 'Differentiate between worst-case and average-case time complexities.', marks: 2, co: 'CO1', bloom_level: 'Understand', unit: 1 },
        { id: 103, section: 'Part A', question_number: 3, question_text: 'State the Greedy Choice property used in optimization problems.', marks: 2, co: 'CO2', bloom_level: 'Remember', unit: 2 },
        { id: 104, section: 'Part A', question_number: 4, question_text: 'What is overlapping subproblems property in Dynamic Programming?', marks: 2, co: 'CO2', bloom_level: 'Understand', unit: 3 },
        { id: 105, section: 'Part A', question_number: 5, question_text: 'Define NP-Completeness and give two classic NP-Complete problem examples.', marks: 2, co: 'CO3', bloom_level: 'Remember', unit: 5 },
      ];

      const part_b = [
        { id: 201, section: 'Part B', question_number: 6, question_text: 'Explain the Divide-and-Conquer strategy. Write the algorithm for Quick Sort and analyze its best and worst-case time complexities.', marks: 10, co: 'CO1', bloom_level: 'Apply', unit: 1 },
        { id: 202, section: 'Part B', question_number: 7, question_text: 'Describe Prim\'s and Kruskal\'s algorithms for finding Minimum Spanning Trees. Compare their time complexities on dense and sparse graphs.', marks: 10, co: 'CO2', bloom_level: 'Analyze', unit: 2 },
        { id: 203, section: 'Part B', question_number: 8, question_text: 'Solve the 0/1 Knapsack problem using Dynamic Programming for weights [2, 3, 4, 5] and values [3, 4, 5, 6] with capacity 5.', marks: 10, co: 'CO2', bloom_level: 'Apply', unit: 3 },
        { id: 204, section: 'Part B', question_number: 9, question_text: 'Explain the Backtracking algorithm for solving the 8-Queens problem. Draw the state space tree for a 4-Queens instance.', marks: 10, co: 'CO3', bloom_level: 'Analyze', unit: 4 },
      ];

      const questions = [...part_a, ...part_b];
      ...
      return {
        ...
        paper: {
          ...
          bloom_distribution: { Remember: 20, Understand: 20, Apply: 30, Analyze: 30 },
          status: 'draft',
          questions,
          part_a,
          part_b
  ...
  async updateQuestionPaper(paperId: number, payload: any): Promise<any> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/exam/papers/${paperId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async moderateQuestionPaper(paperId: number, status: string, notes?: string): Promise<any> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/exam/papers/${paperId}/moderate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ status, moderator_notes: notes })
      });
  ...
  async validatePaper(paperId: number): Promise<any> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/exam/papers/${paperId}/validate`);
      if (!response.ok) throw new Error('Failed to validate paper');
      return await response.json();
    } catch (error) {
      return { status: 'needs-review', gaps: [] };
    }
  },
  ...
  async getMentorEscalations(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/mentor/escalations`);
      if (!response.ok) throw new Error('Failed to fetch escalations');
      return await response.json();
    } catch (error) {
      return [
        { id: 1, mentee_id: 3, student_name: "C. Dinesh", roll_no: "24CC003", reason: "Attendance below 60%, needs counseling support", escalated_to: "counselor", status: "open", created_at: "2026-07-20T10:30:00" }
      ];
    }
  },
  ...
  async updateEscalationStatus(escalationId: number, status: string): Promise<any> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/mentor/escalation/${escalationId}/status`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  async getMentorStats(): Promise<any> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/mentor/stats`);
      if (!response.ok) throw new Error('Failed to fetch stats');
      return await response.json();
    } catch (error) {
      return { total_mentees: 3, overdue_count: 1, open_escalations: 1, checkins_this_month: 2 };
    }
  },

  async getMentorTasks(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/mentor/tasks`);
      if (!response.ok) throw new Error('Failed to fetch mentor tasks');
      return await response.json();
    } catch (error) {
      return [
        { id: 1, title: 'Check in with DAA students', description: 'Schedule checkins for DAA midterm prep', status: 'pending' },
        { id: 2, title: 'Review mock interview notes', description: 'Review notes for Priya and Dinesh', status: 'completed' }
      ];
    }
  },
  ...
  async createMentorTask(title: string, description: string): Promise<any> {
    try {
      ...
      if (!response.ok) throw new Error('Failed to create mentor task');
      return await response.json();
    } catch (error) {
      return { status: 'success' };
    }
  ...
  async editMentorTask(taskId: number, title: string, description: string): Promise<any> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/mentor/tasks/${taskId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ title, description })
      });
      if (!response.ok) throw new Error('Failed to edit mentor task');
      return await response.json();
    } catch (error) {
      return { status: 'success' };
    }
  ...
  async getFutureNotes(studentId: number): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/mentor/notes/${studentId}`);
      if (!response.ok) throw new Error('Failed to fetch future notes');
      return await response.json();
    } catch (error) {
      return [
        { id: 1, note: 'Discuss career path in AI', created_at: '2026-07-28' }
      ];
    }
  },
  ...
  async addFutureNote(studentId: number, note: string): Promise<any> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/mentor/notes`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ student_id: studentId, note })
      });
      if (!response.ok) throw new Error('Failed to add future note');
      return await response.json();
    } catch (error) {
      return { status: 'success' };
    }
  ...
  async editPlacementDrive(id: number, data: any): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/api/placement/drives/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    if (!response.ok) throw new Error('Failed to edit drive');
    return response.json();
  },

  async getInternships(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/placement/internships`);
  ...
  async editInternship(id: number, data: any): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/api/placement/internships/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    if (!response.ok) throw new Error('Failed to edit internship');
    return response.json();
  },
  ...
  async getAlumniDirectory(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/alumni/directory`);
  ...
  async editAlumni(id: number, data: any): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/api/alumni/directory/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    if (!response.ok) throw new Error('Failed to edit alumni');
    return response.json();
  },

  async getAlumniDonations(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/alumni/donations`);
  ...
  async editAlumniDonation(id: number, data: any): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/api/alumni/donations/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    if (!response.ok) throw new Error('Failed to edit donation');
    return response.json();
  },
  ...
  async getCommitteeTasks(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/events/tasks`);
  ...
  async editCommitteeTask(id: number, data: any): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/api/events/tasks/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  async getLabAssets(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/inventory/assets`);
  ...
  async editLabAsset(id: number, data: any): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/api/inventory/assets/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
  ...
  async getSoftwareLicenses(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/inventory/licenses`);
  ...
  async editSoftwareLicense(id: number, data: any): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/api/inventory/licenses/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
    if (!response.ok) throw new Error('Failed to edit license');
    return response.json();
  },

  async getBookRequisitions(): Promise<any[]> {
    try {
      const response = await fetch(`${API_BASE_URL}/api/inventory/books`);
  ...
  async editBookRequisition(id: number, data: any): Promise<any> {
    const response = await fetch(`${API_BASE_URL}/api/inventory/books/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
