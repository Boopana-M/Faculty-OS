...
source_path = 'c:/Users/GURU GAYATHRI/Downloads/Faculty-OS-master/Faculty-OS-master/frontend/src/pages/FacultyAssistant.tsx'
dest_path = 'c:/Users/GURU GAYATHRI/Downloads/Faculty-OS-master/Faculty-OS-master/frontend/src/components/SharedChatInterface.tsx'

with open(source_path, 'r', encoding='utf-8') as f:
    content = f.read()

# We want to extract the chat logic into SharedChatInterface.tsx
# The easiest way is to use regex or just write the file directly. Since it's complex, I'll write the component template and fill it.

template = """import React, { useState, useRef, useEffect } from 'react';
              ...
              <div className="space-y-2">
                {data.activities.map((act: any, i: number) => (
                  <div key={i} className="flex gap-2 text-xs">
                    <Badge variant="outline" className="shrink-0 bg-paper text-ink">{act.duration}</Badge>
                    <div>
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
        {messages.length === 0 && (
          <div className="flex-1 flex flex-col items-center justify-center text-center max-w-md mx-auto opacity-70">
            <Seal agentId={agentId as any} size="lg" />
            ...
            <div className="flex flex-col gap-1 w-full">
              ...
              {(streamingText || isLoading) && (
                <div className="text-sm p-4 rounded-2xl rounded-tl-sm bg-paper text-ink border border-border shadow-sm">
                  {streamingText ? (
                    <ReactMarkdown className="prose prose-sm dark:prose-invert max-w-none">{streamingText}</ReactMarkdown>
                  ) : (
...
print("Created SharedChatInterface.tsx")
